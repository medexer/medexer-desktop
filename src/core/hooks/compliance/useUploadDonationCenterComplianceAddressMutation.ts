...
export default function useUploadDonationCenterComplianceAddressMutation() {
	const {setStep} = useContext(OnboardingContext);

	const {setDonationCenterComplianceDetails} = useAppContext();

	return useMutation({
		mutationFn: async (values: DonationCenterComplianceAddressDTO) => {
			nprogress.start();

			const response =
				await complianceApi.donationCenterControllerUploadComplianceAddress(
					values
				);
		...
		onSuccess(data: DonationCenterComplianceInfo) {
			console.log(
				'UPLOAD-DONATION-CENTER-COMPLIANCE-ADDRESS-SUCCESS :: ',
				data
			);
            ...
            showNotification({
				color: 'green',
				title: 'Success',
				message: 'Center address uploaded successfully',
			});
