...
import useGetAppointmentsHistoryQuery from '@/core/hooks/appointments/useGetAppointmentsHistoryQuery';
import RenderCompletedAppointmentsSection from '@/components/sections/render-completed-appointments-section';

const CompletedAppointmentsTab = () => {
	useGetAppointmentsHistoryQuery();

	const {appointmentsHistory} = useAppContext();
	...
	const filteredAppointments = useMemo(() => {
		if (!appointmentsHistory) return [];

		const normalizedSearchTerm = searchTerm.replace('#', '').toLowerCase();

		return (appointmentsHistory ?? []).filter(
	...
	}, [appointmentsHistory, searchTerm]);
	...
	return (
		...
		>
			...
			<Divider />
			<Box>
				<RenderCompletedAppointmentsSection
					activeTab={activeTab}
...
export default CompletedAppointmentsTab;
